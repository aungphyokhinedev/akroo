  main() {
  print(Rabbit.zg2uni('သီဟိုဠ္မွ ဉာဏ္ႀကီးရွင္သည္ အာယုဝၯနေဆးၫႊန္းစာကို ဇလြန္ေဈးေဘး ဗာဒံပင္ထက္ အဓိ႒ာန္လ်က္ ဂဃနဏဖတ္ခဲ့သည္။'));
}
...
static String  zg2uni(output) {
  ...
  return replace_with_rule(rule,output);
    ...
    static String replace_with_rule(List<Map<String,String>> rule, String output)
    {
        ...
        for(int i=0; i < rule.length; i++) {
          ...
          if(to.contains('\$')){
            var regExp = new RegExp(from);
            Iterable<Match> matches = regExp.allMatches(output);
            ...
            for (var m in matches) {
              ...
              for(int i=0; i< match.length; i++){
                print(i);
                var reg = '\$' + (i+1).toString();
               // to = to.replaceAll(reg, match[i]);
                print(reg);
              }
              to = '(${match})' + to;
            }
          ...
          output = output.replaceAll(new RegExp(from), to);
